...
export default function Hero() {
  ...
  const [heroFeature_demo, setherFearturedemo] = useState({});
  ...
  useEffect(() => {
    const fetchHeader = async () => {
      try {
        ...
        // New array fields
        setHeroDesignString1(data.hero_design_string1 || []);
        
        setHeroDesignString2(data.hero_design_string2 || []);
        // setHeroFeature(data.hero_feature || [{ key: "", value: "" }]);
        const transformedFeatures = (data.hero_feature || []).map(featurea => ({
          icon: featurea.key,
          text: featurea.value
        }));
        
        setHeroFeature(transformedFeatures);
        ...
        <div className="space-y-8">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold text-foreground leading-tight">
              ...
              <TypedText id="typed-text" strings={heroDesignString1} />
            </span> */}
            {heroDesignString1.length > 0 && (
              <span className="text-primary block">
                <TypedText id="typed-text" strings={heroDesignString1} />
              </span>
            )}
            ...
            <h2 className="text-4xl md:text-6xl font-bold text-foreground leading-tight">
              {/* <span className="text-primary block">
              <TypedText id="typed-text" strings={heroDesignString2} />
            </span> */}
            {heroDesignString2.length > 0 && (
              <span className="text-primary block">
                <TypedText id="typed-text" strings={heroDesignString2} />
              </span>
            )}
